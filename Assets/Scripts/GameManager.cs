...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField]
    GameObject player1TutorialUI;

    [SerializeField]
    GameObject player2TutorialUI;
    ...
    [SerializeField]
    GameObject zombieSpawnNorth;

    [SerializeField]
    GameObject zombieSpawnSouth;

    [SerializeField]
    GameObject zombieSpawnWest;

    [SerializeField]
    GameObject zombieSpawnWest1;

    [SerializeField]
    GameObject zombieSpawnWest2;

    [SerializeField]
    GameObject zombieSpawnEast;
    ...
    [SerializeField]
    AudioClip[] terrenceVoice;

    [SerializeField]
    AudioClip[] phillipVoice;

    [SerializeField]
    AudioClip[] joshVoice;

    [SerializeField]
    AudioClip[] altTerrenceVoice;

    [SerializeField]
    AudioClip[] altPhillipVoice;
    ...
    public void ClickedMainMenu()
    {
        ...
        TextMeshProUGUI[] gameCompletedTexts =
            gameCompleted.GetComponentsInChildren<TextMeshProUGUI>(true);
        ...
        foreach (GameObject obj in allGameObjects)
        {
            if (obj.name.StartsWith("ZombieSpawner"))
            {
                ...
                obj.tag.Equals("Zombie")
                || obj.name.Contains("Bullet")
                || obj.name.Contains("Grenade")
            )
            {
                Destroy(obj);
            }
    ...
    void Update()
    {
        if (helicopter.GetComponent<StartHeliProgress>().gameCompleted == true)
        {
            ...
            player1.GetComponent<PlayerHealthSystem>().currentHealth <= 0
            && player2.GetComponent<PlayerHealthSystem>().currentHealth <= 0
        )
        {
            GameOver();
        }
        else
        {
            if (!inGame)
            {
                ...
                {
                    if (
                        !p1Movement.isAutoMoving
                        && !p2Movement.isAutoMoving
                        && speechKey >= 0
                        && speechKey <= 19
                    )
                    {
            ...
            else
            {
                ...
                if (
                    ((!level1Done && !level2Done) || (level1Done && level2Done))
                ...
                if (
                    ...
                    && !level1Done
                    && !level2Done
                    && !player1.GetComponent<PlayerHealthSystem>().isDowned
                    && !player2.GetComponent<PlayerHealthSystem>().isDowned
                )
                {
                    mission.SetActive(false);
                    inGame = false;
                ...
                else if (!player1Dead && !player2Dead)
                {
                    ...
                    )
                    {
                        ...
                        && level2Done
                        && allGearsCollected
                        && helicopter.GetComponent<FixProgress>().completed
                        && !helicopter.GetComponent<StartHeliProgress>().completed
                        && (speechKey == 44 || speechKey == 45)
                    )
                    {
                        if (!isPaused)
                        {
                    ...
                    {
                        missionText.text =
                            "<b>Mission:</b>\n[Phillip] Fix the helicopter. ("
                            + (4 - player2.GetComponent<PlayerInventory>().gearCount)
                ...
                else if (level1Done && !level2Done)
                {
                    missionText.text =
                        "<b>Mission:</b>\nKill all 80 zombies. ("
                        + (p1ZombieKilled + p2ZombieKilled - 30)
                        + "/80)\nBlow up the 4 cars using grenades to get gears. ("
                        + (4 - carCount)
                        + "/4)\n[Phillip] Collect all 4 gears. ("
                        + player2.GetComponent<PlayerInventory>().gearCount
                        + "/4)";
                }
            }
    ...
    public void DestroyDrops()
    {
        ...
        {
            if (
                obj.tag.Equals("GrenadeDrop")
                || obj.tag.Equals("HealthDrop")
                || obj.tag.Equals("GearDrop")
            )
            {
                Destroy(obj);
            }
